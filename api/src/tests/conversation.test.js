...
describe('User logged in with JWT', () => {
  ...
  it('Should return all user conversations', async () => {
    const res = await request(app)
      .get(`/api/user/${user_id}/conversations`)
      .set('Authorization', `Bearer ${token}`);
  ...
  it('Should return one user conversation', async () => {
    const res = await request(app)
      .get(`/api/user/${user_id}/conversation/${conversation_id}`)
      .set('Authorization', `Bearer ${token}`);
  ...
  it('Should update one user conversation', async () => {
    const res = await request(app)
      .put(`/api/user/${user_id}/conversation/${conversation_id}`)
      .set('Authorization', `Bearer ${token}`)
      .send({ blocked: true });
  ...
  it('Should delete one user conversation', async () => {
    const res = await request(app)
